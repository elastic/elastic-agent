...
const indexingTimeout = 5 * time.Minute
...
func TestFQDN(t *testing.T) {
	...
	verifyHostNameInIndices(t, "logs-*", shortName, startedAt, info.Namespace, info.ESClient, indexingTimeout)
	verifyHostNameInIndices(t, "metrics-*", shortName, startedAt, info.Namespace, info.ESClient, indexingTimeout)
	...
	for name, runtime := range runtimes {
		t.Run(name, func(t *testing.T) {
			...
			verifyHostNameInIndices(t, "logs-*", fqdn, since, info.Namespace, info.ESClient, indexingTimeout)
			verifyHostNameInIndices(t, "metrics-*", fqdn, since, info.Namespace, info.ESClient, indexingTimeout)
		})
	}
