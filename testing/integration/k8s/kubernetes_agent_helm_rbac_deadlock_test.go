...
const (
	...
	rbacDeadlockInitialPeriod            = 10 * time.Second
	rbacDeadlockUpdatedPeriod            = 20 * time.Second
)
...
func TestKubernetesAgentHelmRBACDeadlock(t *testing.T) {
	...
	_ = info // used when t.Skip is removed
	t.Skip("test is meant to be run occasionally, not every run. Enable when CI will be ready to handle this condition")
...
func kubernetesPackagePolicyRequest(agentPolicyID, version string, period time.Duration) kibana.PackagePolicyRequest {
...
func kubernetesPackagePolicyInputs(period time.Duration) []map[string]interface{} {
			...
			"streams": []map[string]interface{}{
				{
					...
					"vars": map[string]interface{}{
						...
						"period":            map[string]interface{}{"type": "text", "value": period.String()},
...
func readAgentID(
	...
	id, err := k8sGetAgentID(ctx, kCtx.client, &stdout, &stderr, namespace, podName, rbacDeadlockAgentContainer)
	require.NoError(t, err, "failed to read Elastic Agent status")
	require.NotEmpty(t, id, "Elastic Agent status has no agent ID")
	return id
}
		...
		"query": map[string]any{
			"bool": map[string]any{
				"filter": []any{
					...
					map[string]any{"term": map[string]any{"metricset.period": rbacDeadlockUpdatedPeriod.Milliseconds()}},
				},
			},
		},
	}
