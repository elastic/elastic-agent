...
func TestDuplicateAgentUpgradeAction(t *testing.T) {
	info := define.Require(t, define.Requirements{
		Group: Fleet,
		Stack: &define.Stack{},
		Local: false, // requires Agent installation
	...
	// Upgrade to a very same version.
	...
	t.Logf("Installing Elastic Agent (privileged)...")
	...
	t.Log("Waiting for upgrade details to show up in Fleet")
	...
	t.Log("Waiting for agent to have empty upgrade details")
	require.Eventuallyf(t, func() bool {
		agent, err = kibClient.GetAgent(ctx, kibana.GetAgentRequest{ID: agentID})
		return err == nil && agent.UpgradeDetails == nil
	},
	...
	t.Logf("Duplicate upgrade from version \"%s-%s\" to version \"%s-%s\"...",
		startParsedVersion, startVersionInfo.Binary.Commit,
		endVersionInfo.Binary.String(), endVersionInfo.Binary.Commit)
	err = fleettools.UpgradeAgent(ctx, kibClient, agentID, endVersionInfo.Binary.String(), true)
	require.NoError(t, err)
