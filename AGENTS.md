...
- **Lint scope:** **`mage check:lint`** / **`make lint`** — changed files; **`mage check:lintAll`** / **`make lint-all`** — entire codebase. Both run golangci-lint once per build-tag set (`""`, `local`, `define`, each combined with `integration,requirefips,kubernetes_inner,mage`) so files gated behind those tags actually get linted; see `buildTagSets` in [dev-tools/mage/lint.go](./dev-tools/mage/lint.go) and the matching `tagset` matrix in [.github/workflows/golangci-lint.yml](./.github/workflows/golangci-lint.yml). **`mage check:all`** is separate: license headers, integration-test `define.Require` validation, and docs path validation (see [magefile.go](./magefile.go) `Check.All`).
