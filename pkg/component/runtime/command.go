			...
			if c.actionState == actionStart {
				if c.proc == nil {
					...
					if now.Sub(c.lastCheckin) <= checkinPeriod {
						...
						c.missedCheckins++
						_, _ = c.logStd.Write([]byte(fmt.Sprintf("Last check-in was: %s, now is: %s. The diff %s is higher than allowed %s.", c.lastCheckin.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), now.Sub(c.lastCheckin), checkinPeriod)))
					}
