...
func TestCheckinCompress(t *testing.T) {
	tcs := []struct {
		...
		name: "checkin compress enabled with explicit threshold",
		yaml: `
agent:
  features:
    checkin_compress:
      ...
      threshold_size: 2048`,
		...
		name: "checkin compress enabled uses default threshold",
		yaml: `
agent:
  features:
    checkin_compress:
      enabled: true`,
		...
		name: "checkin compress disabled with threshold zero",
		yaml: `
agent:
  features:
    checkin_compress:
      ...
      threshold_size: 0`,
		...
		name: "checkin compress absent uses defaults",
		yaml: `
agent:
  features:`,
		wantEnabled:   false,
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotEnabled, gotThreshold := CheckinCompress()
			require.Equal(t, tc.wantEnabled, gotEnabled)
			require.Equal(t, tc.wantThreshold, gotThreshold)
		})
	}
}
