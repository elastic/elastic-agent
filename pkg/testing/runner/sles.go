...
// SlesRunner is a handler for running tests on SUSE Linux Enterpriser Server
type SlesRunner struct{}
...
func (SlesRunner) Prepare(ctx context.Context, sshClient SSHClient, logger Logger, arch string, goVersion string) error {
	...
	zypperCtx, zypperCancel := context.WithTimeout(ctx, 20*time.Minute)
	defer zypperCancel()
	stdOut, errOut, err := sshClient.ExecWithRetry(zypperCtx, "sudo", []string{"zypper", "install", "-y ", "-t", "pattern", "devel_basis"}, 15*time.Second)
	if err != nil {
...
func (SlesRunner) Copy(ctx context.Context, sshClient SSHClient, logger Logger, repoArchive string, builds []Build) error {
	return linuxCopy(ctx, sshClient, logger, repoArchive, builds)
}

// Run the test
func (SlesRunner) Run(ctx context.Context, verbose bool, sshClient SSHClient, logger Logger, agentVersion string, prefix string, batch define.Batch, env map[string]string) (OSRunnerResult, error) {
	var tests []string
...
func (SlesRunner) Diagnostics(ctx context.Context, sshClient SSHClient, logger Logger, destination string) error {
