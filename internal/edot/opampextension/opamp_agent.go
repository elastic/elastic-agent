...
func (o *opampAgent) statusAggregatorEventLoop(unsubscribeFunc status.UnsubscribeFunc, statusChan <-chan *status.AggregateStatus) {
	...
	for {
		...
		case statusUpdate, ok := <-statusChan:
			...
			// Compare the full health tree (not just the top-level status) so that
			// sub-component status changes are not swallowed when a higher-priority
			// component already dominates the top-level aggregate status.
			if proto.Equal(lastHealth, componentHealth) {
				continue
			}
			lastHealth = componentHealth
