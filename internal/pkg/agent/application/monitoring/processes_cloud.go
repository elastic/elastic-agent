...
func expectedCloudProcessID(binaryName string, componentID string) string {
	...
	if strings.Contains(binaryName, "apm-server") {
		// cloud understands `apm-server-default` and does not understand `apm-default`
		return strings.Replace(componentID, "apm-", "apm-server-", 1)
	}

	return componentID
}

func matchesCloudProcessID(binaryName string, currentComponentID string, id string) bool {
