...
func (c *Coordinator) watchRuntimeComponents(ctx context.Context) {
	...
	var otelComponentChan <-chan runtime.ComponentComponentState
	...
	otelComponentChan = c.managerChans.otelManagerComponentUpdate
	for {
		select {
		case <-ctx.Done():
			return
		...
		case componentState := <-otelComponentChan:
			logComponentStateChange(c.logger, state, &componentState)
			// Forward the final changes back to Coordinator, unless our context
			// has ended.
			select {
