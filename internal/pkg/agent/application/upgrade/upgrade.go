...
func (u *Upgrader) Upgrade(ctx context.Context, version string, sourceURI string, action *fleetapi.ActionUpgrade, det *details.Details, skipVerifyOverride bool, skipDefaultPgp bool, pgpBytes ...string) (_ reexec.ShutdownCallbackFn, err error) {
	...
	previousParsedVersion := u.getParsedVersionWithFallback(release.VersionWithSnapshot(), agtversion.NewParsedSemVer(8, 13, 0, "", ""))
...
func (u *Upgrader) getParsedVersionWithFallback(currentVersion string, fallbackVersion *agtversion.ParsedSemVer) *agtversion.ParsedSemVer {
	previousParsedVersion, err := agtversion.ParseVersion(currentVersion)
	if err != nil {
		previousParsedVersion = fallbackVersion
		u.log.Warnw(fmt.Sprintf("parsing of current version failed, watcher selection will fall back to comparing to %q", previousParsedVersion), "error.message", err)
	}
