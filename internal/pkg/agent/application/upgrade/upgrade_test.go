	...
	testCases := map[string]testCase{
		...
		"should return error and cleanup downloaded artifact and extracted archive if markUpgrade fails": {
			...
			upgraderMocker: func(upgrader *Upgrader, archivePath string, versionedHome string) {
				...
				upgrader.markUpgrade = func(log *logger.Logger, dataDirPath string, updatedOn time.Time, agent, previousAgent agentInstall, action *fleetapi.ActionUpgrade, upgradeDetails *details.Details, _ map[string]ttl.TTLMarker) error {
					return testError
				}
			},
			checkArchiveCleanup:       true,
