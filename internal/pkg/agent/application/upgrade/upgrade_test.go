...
func Test_getParsedVersionWithFallback(t *testing.T) {

	v123SnapshotVersion, _ := agtversion.ParseVersion("1.2.3-SNAPSHOT")
	v456 := agtversion.NewParsedSemVer(4, 5, 6, "", "")

	type args struct {
		currentVersion  string
		fallbackVersion *agtversion.ParsedSemVer
	}

	tests := []struct {
		name    string
		args    args
		want    *agtversion.ParsedSemVer
		wantLog bool
	}{
		{
			name: "Happy path - valid input version",
			args: args{
				currentVersion:  v123SnapshotVersion.Original(),
				fallbackVersion: v456,
			},
			want: v123SnapshotVersion,
		},
		{
			name: "Fallback - invalid input version, fallback is returned",
			args: args{
				currentVersion:  "asd.asd.asd",
				fallbackVersion: v456,
			},
			want:    v456,
			wantLog: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			log, obs := logger.NewTesting(tt.name)
			assert.Equalf(t, tt.want, getParsedVersionWithFallback(log, tt.args.currentVersion, tt.args.fallbackVersion), "getParsedVersionWithFallback(%v, %v, %v)", log, tt.args.currentVersion, tt.args.fallbackVersion)
			if tt.wantLog {
				assert.NotEmpty(t, obs.FilterMessageSnippet(tt.args.fallbackVersion.String()).All())
			} else {
				assert.Empty(t, obs.FilterMessageSnippet(tt.args.fallbackVersion.String()).All())
			}
		})
	}
}
