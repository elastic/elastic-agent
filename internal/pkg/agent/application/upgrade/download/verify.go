...
func VerifyChecksum(hasher hash.Hash, filename, checksumFileName string) error {
	...
	if _, err := io.Copy(hasher, f); err != nil {
		return fmt.Errorf("failed to read file to calculate hash")
	}
...
func FetchPGPKeys(log *logger.Logger, config *Config, defaultPGPKey []byte, skipDefaultPGP bool, pgpSources []string) ([][]byte, error) {
	var pgpKeys [][]byte
...
func FetchPGPSignature(ctx context.Context, log *logger.Logger, config *Config, src string) ([]byte, error) {
	if IsLocal(src) {
...
func Verify(ctx context.Context, log *logger.Logger, config *Config, defaultPGP []byte, src, dst string, skipDefaultPgp bool, pgpBytes ...string) (err error) {
	if err = VerifySHA512Hash(dst); err != nil {
		return fmt.Errorf("failed to verify checksum: %w", err)
	}

	var signature []byte
	if signature, err = FetchPGPSignature(ctx, log, config, src+ascSuffix); err != nil {
