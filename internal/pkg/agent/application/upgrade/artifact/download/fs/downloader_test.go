	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				assert.DirExists(t, targetDirPath, "downloader should not clean up target directory on error")
				return
			}
...
func TestDownloader_downloadFile(t *testing.T) {
	...
	err := e.downloadFile("elastic-agent-1.2.3-linux-x86_64.tar.gz", filepath.Join(targetDirPath, "elastic-agent-1.2.3-linux-x86_64.tar.gz"))
