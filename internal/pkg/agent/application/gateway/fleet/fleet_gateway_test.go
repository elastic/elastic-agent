...
func TestFleetGatewayCheckinResponseMissingActionsField(t *testing.T) {
	...
	gateway, err := newFleetGatewayWithScheduler(log, settings, agentInfo, client, scheduler, noop.New(), emptyStateFetcher, stateStore)
	require.NoError(t, err)
	...
	waitFn := ackSeq(
		client.Answer(func(_ http.Header, _ io.Reader) (*http.Response, error) {
