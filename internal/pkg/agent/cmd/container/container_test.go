...
func TestContainerTestPaths(t *testing.T) {
	cases := map[string]struct {
		config   string
		expected common.ContainerPaths
	}{
		"only_state_path": {
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			var paths common.ContainerPaths
...
func TestKibanaFetchToken(t *testing.T) {
	t.Run("should fetch details from items in the api response", func(t *testing.T) {
		...
		ak, err := kibanaFetchToken(common.SetupConfig{Kibana: common.KibanaConfig{RetryMaxCount: 1}}, client, &policy, cli.NewIOStreams(), "tokenName")
		require.NoError(t, err)
...
func TestShouldEnroll(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"should not enroll if fleet enroll is disabled": {
			cfg:                  common.SetupConfig{Fleet: common.FleetConfig{Enroll: false}},
			expectedShouldEnroll: false,
		},
		"should enroll if fleet force is true": {
			cfg:                  common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, Force: true}},
		...
		"should enroll on agent id but no existing id": {
			...
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1", ID: "diff-agent-id"}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
  ...
  protocol: "https"`)), nil).Once()
		...
		"should enroll on fleet url change": {
			statFn: func(path string) (os.FileInfo, error) { return nil, nil },
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1"}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
				m := mockStorage.NewStorage(t)
				m.On("Load").Return(io.NopCloser(strings.NewReader(`fleet:
  enabled: true
  ...
  protocol: "https"`)), nil).Once()
		...
		"should enroll on fleet token change": {
			statFn: func(path string) (os.FileInfo, error) { return nil, nil },
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1", EnrollmentToken: enrollmentTokenOther}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
				m := mockStorage.NewStorage(t)
				m.On("Load").Return(io.NopCloser(strings.NewReader(`fleet:
  enabled: true
  ...
  protocol: "https"`)), nil).Once()
		...
		"should enroll on unauthorized api": {
			statFn: func(path string) (os.FileInfo, error) { return nil, nil },
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1", EnrollmentToken: enrollmentToken}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
				m := mockStorage.NewStorage(t)
				m.On("Load").Return(io.NopCloser(strings.NewReader(`fleet:
  enabled: true
  ...
  protocol: "https"`)), nil).Once()
		...
		"should not enroll on no changes with agent ID and replace token": {
			statFn: func(path string) (os.FileInfo, error) { return nil, nil },
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1", ID: "custom-id", EnrollmentToken: enrollmentToken, ReplaceToken: replaceToken}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
				m := mockStorage.NewStorage(t)
				m.On("Load").Return(io.NopCloser(strings.NewReader(`fleet:
  enabled: true
  ...
  protocol: "https"`)), nil).Once()
		...
		"should not update the enrollment token hash if it does not exist in setup configuration": {
			statFn: func(path string) (os.FileInfo, error) { return nil, nil },
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1", EnrollmentToken: ""}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
				m := mockStorage.NewStorage(t)
				m.On("Load").Return(io.NopCloser(strings.NewReader(`fleet:
  enabled: true
  ...
  protocol: "https"`)), nil).Once()
		...
		"should not update the replace token hash if it does not exist in setup configuration": {
			statFn: func(path string) (os.FileInfo, error) { return nil, nil },
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1", EnrollmentToken: "", ReplaceToken: ""}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
				m := mockStorage.NewStorage(t)
				m.On("Load").Return(io.NopCloser(strings.NewReader(`fleet:
  enabled: true
  ...
  protocol: "https"`)), nil).Once()
		...
		"should not enroll on no changes and update the stored enrollment and replace token hash": {
			statFn: func(path string) (os.FileInfo, error) { return nil, nil },
			cfg:    common.SetupConfig{Fleet: common.FleetConfig{Enroll: true, URL: "host1", EnrollmentToken: enrollmentToken, ReplaceToken: replaceToken}},
			encryptedDiskStoreFn: func(t *testing.T, savedConfig *configuration.Configuration) storage.Storage {
				m := mockStorage.NewStorage(t)
				m.On("Load").Return(io.NopCloser(strings.NewReader(`fleet:
  enabled: true
