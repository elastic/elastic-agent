...
func unprivilegedCmd(streams *cli.IOStreams, cmd *cobra.Command) (err error) {
	...
	if daemonTimeout >= 0 {
		...
		ctx := common.HandleSignal(context.Background()) // allowed to be cancelled
		err = wait.ForAgent(ctx, common.DaemonTimeout)
		if err != nil {
...
func ensureNoServiceComponentIssues() error {
	...
	comps, err := inspect.GetComponentsFromPolicy(ctx, l, paths.ConfigFile(), 0, forceNonRoot)
	if err != nil {
