...
type Option struct {
	...
	KeyLength       int
	SaltLength      int
	IVLength        int // deprecated - setting is ignored
...
// nonceLen is the length of a nonce generated by https://pkg.go.dev/crypto/cipher#NewGCMWithRandomNonce
// which is 96 bits, or 12 bytes.
const nonceLen = 12
...
// Data block structure is:
//
//	IV | ENCRYPTED-BYTES-LENGTH (little endian, uint32) | ENCRYPTED-BYTES
//
// This differs from the automatic handling that NewGCMWithRandomNonce does which is: IV | ENCRYPTED-BYTES
...
// readTo decrypts data into the passed slice.
...
// consumeBlock consumes an encrypted data block.
// It handles parsing the v2 block header correctly to decrypt data.
...
// readBlockInfo reads a v2 data block header and returns the IV and encrypted data length.
func (r *Reader) readBlockInfo() ([]byte, int, error) {
	buf := make([]byte, nonceLen+4)
	...
	iv := buf[:nonceLen]
	l := binary.LittleEndian.Uint32(buf[nonceLen:])
