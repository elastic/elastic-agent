...
func getReceiverTypeForComponent(comp *component.Component) (otelcomponent.Type, error) {
	...
	switch beatName {
	case "filebeat":
		// Note: do not import "github.com/elastic/beats/v7/x-pack/filebeat/fbreceiver" to use fbreceiver.Name, by doing so the binary gets heavy
		return otelcomponent.MustNewType("filebeatreceiver"), nil
	case "metricbeat":
		// Note: do not import "github.com/elastic/beats/v7/x-pack/filebeat/mbreceiver" to use mbreceiver.Name, by doing so the binary gets heavy
