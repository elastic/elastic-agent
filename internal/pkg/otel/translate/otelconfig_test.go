...
func TestGetReceiversConfigForComponentHostnameOverride(t *testing.T) {
	comp := &component.Component{
	...
	t.Run("env_var_set", func(t *testing.T) {
		...
		for id, raw := range result {
			...
			assert.Equal(t, "override-node", cfg["hostname"],
				"receiver %s: hostname not injected into shared config", id)
		}
	})

	t.Run("env_var_unset", func(t *testing.T) {
		result, err := getReceiversConfigForComponent(comp, &info.AgentInfo{}, nil)
		require.NoError(t, err)
		require.NotEmpty(t, result)
