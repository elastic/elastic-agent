...
func newSubprocessExecution(collectorPath string, enablePartialReload bool) (*subprocessExecution, error) {
	args := []string{
...
func (r *subprocessExecution) startCollector(
	...
	agentLogger.Infof("supervised collector started with pid: %d", processInfo.Process.Pid)

	ctl := newProcHandle(processInfo, agentLogger, collectorLevel,
