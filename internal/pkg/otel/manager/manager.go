...
func NewOTelManager(logger *logger.Logger) (*OTelManager, error) {
	...
	for {
		...
		case err = <-processErrCh:
			if err == nil {
				...
				ctl, err = startSupervisedCollector(ctx, m.logger, m.collectorBinaryPath, m.collectorBinaryArgs, m.cfg, processErrCh, m.statusCh)
				if err != nil {
...
// clearStatus clears the status channel
func (m *OTelManager) clearStatus(ctx context.Context) {
	select {
	case m.statusCh <- nil:
	case <-ctx.Done():
	}
