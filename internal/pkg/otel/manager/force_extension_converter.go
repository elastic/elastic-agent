...
func (fe *forceExtension) Convert(ctx context.Context, conf *confmap.Conf) error {
	err := func() error {
		...
		if err != nil {
			// no service defined in the configuration, this is going to error by the collector any way
			// no reason to pollute with more error information that is not really related to the issue at the moment
			return nil
		}
