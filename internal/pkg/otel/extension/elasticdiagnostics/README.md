...
- elastic-agent's client for this route is `otel.PerformActionExt` in [../../actions.go](../../actions.go), used by `Coordinator.PerformAction` when a component's `RuntimeManager` is `otel`.
