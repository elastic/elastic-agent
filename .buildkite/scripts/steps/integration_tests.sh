...
TESTS_EXIT_STATUS=$?
AGENT_VERSION="${OVERRIDE_TEST_AGENT_VERSION}" STACK_PROVISIONER="ess" TEST_INTEG_CLEAN_ON_EXIT=true SNAPSHOT=true mage integration:test
TESTS_EXIT_STATUS_SERVERLESS=$?
set -e
...
exit $TESTS_EXIT_STATUS + $TESTS_EXIT_STATUS_SERVERLESS
