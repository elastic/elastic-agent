...
function ess_up() {
  ...
  if [ -z "$STACK_VERSION" ]; then
    echo "Error: Specify stack version: ess_up [stack_version] [stable_snapshot_version]" >&2
    return 1
  fi
  
  if [ -z "$STABLE_SNAPSHOT_VERSION" ]; then
    echo "Error: Specify stable snapshot version: ess_up [stack_version] [stable_snapshot_version]" >&2
    return 1
  fi

  echo "~~~ Starting ESS. Version: ${STACK_VERSION}. Stable snapshot: ${STABLE_SNAPSHOT_VERSION}"  
  ...
  terraform apply \
    ...
    -var="stable_snapshot_version=${STABLE_SNAPSHOT_VERSION}" \
