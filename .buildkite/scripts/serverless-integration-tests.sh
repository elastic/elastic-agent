...
export ELASTICSEARCH_HOST=$(vault kv get -field=es_host kv/ci-shared/observability-ingest/elastic-agent-serverless-staging-to-be-deleted)
export ELASTICSEARCH_USERNAME=$(vault kv get -field=es_username kv/ci-shared/observability-ingest/elastic-agent-serverless-staging-to-be-deleted)
export ELASTICSEARCH_PASSWORD=$(vault kv get -field=es_password kv/ci-shared/observability-ingest/elastic-agent-serverless-staging-to-be-deleted)
export KIBANA_HOST=$(vault kv get -field=kibana_endpoint kv/ci-shared/observability-ingest/elastic-agent-serverless-staging-to-be-deleted)
