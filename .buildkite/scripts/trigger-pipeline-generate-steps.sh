...
readarray -t ELIGIBLE_BRANCHES < <(curl --retry-all-errors --retry 5 --retry-delay 5 -fsSL $ACTIVE_BRANCHES_URL)
if [[ $? -ne 0 ]]; then
  echo "There was an error downloading the branch list from [$ACTIVE_BRANCHES_URL]. Exiting."
  exit 1
fi
set -e
