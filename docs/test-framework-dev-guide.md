...
- `EXTERNAL=true|false`: If `false`, don't download any components; `agentbeat` is instead built from a `beats` repository checked out next to this one. For a fully local dependency set (for example a custom endpoint build), also set `USE_PACKAGE_VERSION=false`. Default is `true`.
