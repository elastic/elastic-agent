...
// of the settings. It sets Build.Snapshot, Build.AgentCoreVersion, Build.DependenciesVersion, and
// Packaging.Manifest. Build.AgentCoreCommitHash is set only when CoreSource is CoreSourceManifest:
// for CoreSourceLocal the locally-built binary's commit is git HEAD, not the manifest's.
// It is a no-op if ManifestURL is empty.
